...
CKEDITOR.plugins.add( 'footnotes', {
    ...
    buildFootnote: function(footnote_id, footnote_text, data, editor) {
        ...
        if (data && data.occurrences[footnote_id] == 1) {
            links = '<a href="#footnote-marker' + prefix + '-' + order + '-1" title="각주 위치로 돌아가기">^</a> ';
        } else if (data && data.occurrences[footnote_id] > 1) {
            var i = 0
              , l = data.occurrences[footnote_id]
              , n = l;
            for (i; i < l; i++) {
              links += '<a href="#footnote-marker' + prefix + '-' + order + '-' + (i + 1) + '" title="각주 위치로 돌아가기">' + letters.charAt(i) + '</a>';
                if (i < l-1) {
                    links += ', ';
                } else {
                    links += ' ';
    ...
    addFootnote: function(footnote, editor) {
        ...
        if ($footnotes.length == 0) {
            var container = '<section class="footnotes"><header><h2>각주</h2></header><ol>' + footnote + '</ol></section>';
    ...
    reorderMarkers: function(editor) {
        ...
        $markers.each(function(){
            ...
            var marker = '<a href="#footnote' + prefix + '-' + n + '" id="footnote-marker' + prefix + '-' + marker_ref + '" rel="footnote" title="각주 보기">[' + n + ']</a>';
