...
$(document).ready(function() {
  ...
  $('.ib01 .slide').on('init', function(){  //2021.8.3 웹접근성 작업, 2022.10.06
    $('.slick-dots').append($('<li class="pause"><a title="일시정지하기" class="pause"><i class="xi-pause"></i></a><a title="재생하기" class="play"><i class="xi-play"></i></a></li>'));
    setTimeout(function(){
  ...
  if($("span.cavacnotify").length > 0) $("span.cavacnotify").cavacnotify();
	...
	//검색 페이지 접근시 검색 결과 Ajax
	if($('body').hasClass('page-search')) {
		let status = (window.location.pathname === 'en/search') ? 'keyword' : 'term';
		let searchParam = new URLSearchParams(window.location.search);
		let key = searchParam.get('key');
		$.post('/ajax/webzine', {type:'search', status:status, key:key}).done(function (res) {
			$("div.menu-name-menu-search ul.menu li a:eq(0)").append(' ('+res.keyword+')');
